...
van_dijk_2004 <- which(grepl("VAN DIJK J, 2005, DEEPENING DIVIDE", 
                             CRwos$ref, ignore.case = TRUE))


new_van_dijk_2004 <- CRwos[1772, c("AU", "PY", "DI")]

new_van_dijk_2004 <- data.frame()
for (i in 1:140) {
  new_van_dijk_2004 <- rbind(new_van_dijk_2004, CRwos[1772, c("AU", "PY", "DI")])
}

ref_van2004 <- grepl("VAN DIJK J, 2005, DEEPENING DIVIDE", 
                          CRwos$ref[van_dijk_2004], ignore.case = TRUE)
if (all(ref_van2004)) {
  CRwos[van_dijk_2004, c("AU", "PY", "DI")] <- new_van_dijk_2004
} else {
  CRwos[van_dijk_2004, c("AU", "PY", "DI")]
}
