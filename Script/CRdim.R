...
cr_cold <- paste("field", 1:ncols, sep ="") # create and paste the name of the colums
...
CRsplit_dim[, c(1:9)] <- lapply(CRsplit_dim[, c(1:9)], trimws )

ind_par <- grep("^10", 
                CRsplit_dim$field6) # getting the indices of rows that starts with 10

CRsplit_dim$DI <- NA
CRsplit_dim$DI[ind_par] <- CRsplit_dim$field6[ind_par]

ind_par <- grep("^10", 
                CRsplit_dim$field7) # getting the indices of rows that starts with 10
CRsplit_dim$DI[ind_par] <- CRsplit_dim$field7[ind_par]

ind_par <- grep("^10", 
                CRsplit_dim$field8) # getting the indices of rows that starts with 10
CRsplit_dim$DI[ind_par] <- CRsplit_dim$field8[ind_par]

CRsplit_dim <- CRsplit_dim[, c(1,2,10,9)]
